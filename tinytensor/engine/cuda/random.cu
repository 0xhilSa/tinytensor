...
__device__ inline void mulhilo(uint32_t a, uint32_t b, uint32_t *hi, uint32_t *lo){
  *lo = a * b;
  *hi = __umulhi(a, b);
}

__device__ inline void philox_round(uint32_t ctr[4], uint32_t key[2]){
  uint32_t hi0, lo0;
  uint32_t hi1, lo1;
...
__device__ inline void philox4x32(uint32_t ctr[4], uint32_t key[2]){
  #pragma unroll
  for(int i=0;i<10;i++){
...
__device__ inline float uint32_to_uniform(uint32_t x){
  return x * (1.0f / 4294967296.0f);
}

__global__ void rand_kernel(float *out, uint64_t seed, size_t n){
  ...
  key[1] = seed >> 32;
  philox4x32(ctr, key);
  if(base + 0 < n) out[base + 0] = uint32_to_uniform(ctr[0]);
  if(base + 1 < n) out[base + 1] = uint32_to_uniform(ctr[1]);
  if(base + 2 < n) out[base + 2] = uint32_to_uniform(ctr[2]);
  if(base + 3 < n) out[base + 3] = uint32_to_uniform(ctr[3]);
}
...
//  if(!PyArg_ParseTuple(args,"n|O",&n,&seed_obj)) return NULL;
//  uint64_t seed;
//  if(seed_obj == Py_None){
//    if(global_seed == 0)
...
//  if(cudaMalloc(&s->ptr, bytes) != cudaSuccess){
//    free(t);
//    free(s);
...
//  t->device.type = CUDA;
//  t->device.index = 0;
//  t->shape = (size_t*)malloc(sizeof(size_t));
//  t->stride = (size_t*)malloc(sizeof(size_t));
//  if(!t->shape || !t->stride){
//    destroy(t);
...
//    rand_kernel<<<blocks,threads>>>((float*)s->ptr,seed,n);
...
static PyObject *rand_(PyObject *self, PyObject *args){
  ...
  size_t bytes = N*sizeof(float32);
  float *dptr;
  cudaError_t err = cudaMalloc((void**)&dptr, bytes);
  if(err != cudaSuccess){
    free(shape); free(stride); free(s); free(t);
    PyErr_SetString(PyExc_RuntimeError,"cudaMalloc failed");
    return NULL;
  ...
  rand_kernel<<<blocks, threads>>>(dptr, N, seed);
  cudaDeviceSynchronize();
  return PyCapsule_New(t,"tensor_t on CUDA",capsule_destroyer);
}
